...
# USAGE:       check_tie_connection wire_length_limitation
# FOR:         GE-02-12
########################################################################

#set FOOTPRINT_TIE "TIE"
proc check_tie_connection { { wire_length_limitation 100 } { output check_tie_connection.rep } } {
	...
	set tie_cell [ get_cells -hier -filter "ref_name =~ *${FOOTPRINT_TIE}*" -quiet ]
  ...
  set fanout_order ""
	...
	foreach tie_name $tie_names {
		...
		set tie_pin [ get_attr [ get_pins -of [ get_cells $tie_name ] -leaf -filter "pin_direction == out" ] full_name ]
    ...
    set length [ NET_LENGTH $net ]
	  ...
	  if { $length > $wire_length_limitation } {
		...
		if { $load_list_length > 16 } {
			set fan($tie_pin) $load_list_length
			lappend tie_pin_list $tie_pin
			incr over16
		}
	}
	...
	foreach pin $tie_pin_list {
		lappend tie_pin_fanout "$pin $fan($pin)"
	}
	set fanout_order [ lsort -real -index 1 -decreasing $tie_pin_fanout ]
	foreach order $fanout_order {
		set pin [ lindex $order 0 ]
		set pin_fanout [ lindex $order 1 ]
		puts $file "Error: $pin ($pin_fanout)"
	}
	...
	puts $file "Total Tie Wire Length over ${wire_length_limitation}(um) = $limitnets_length"
