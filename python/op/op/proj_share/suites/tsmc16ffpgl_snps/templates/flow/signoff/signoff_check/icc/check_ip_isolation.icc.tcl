##################################################################################
# PROGRAM:     check_ip_isolation.icc.tcl
# CREATOR:     Michael Mo <michaelm@alchip.com>
# DATE:        Fri Mar 17 11:38:31 CST 2017
# DESCRIPTION: Check ip wire length in IC Compiler
	...
	foreach ip_reference_name $ip_reference_names {
   	...
   	if { [ sizeof_collection $ip ] == 0 } {
		...
		foreach ip_name $ip_names {
			...
			foreach pin $ip_pins {
				set ip_net [ get_flat_net -of [ get_pins $pin ] -quiet ]
				set connect_cell [ remove_from_collection [ get_flat_cells -of [ get_flat_pins -of [ get_flat_nets $ip_net -quiet ] -quiet ] -quiet ] [ get_cells $ip_name ] ]
