...
report_timing_derate > timing_derate.rpt
...
if {[info exists ENABLE_POCV] && $ENABLE_POCV == "true" } {
    if {[file exists report_pocv_not_annotated.rpt]} {file delete report_pocv_not_annotated.rpt}
    ...
    echo "Read pocvm $pocvWireFile" 	>> report_pocv_not_annotated.rpt
    echo "Read pocvm $pocvWireFile"
    read_aocvm $pocvWireFile 		>> report_pocv_not_annotated.rpt
    foreach voltage [array names MV_LIBRARY_INST_MAP] {
        if { [info exist pocvLibrary($voltage)] && $pocvLibrary($voltage) != "" } {
            foreach file $pocvLibrary($voltage) {
                echo "Read pocvm $file" >> report_pocv_not_annotated.rpt
                echo "Read pocvm $file"
                read_ocvm $file >> report_pocv_not_annotated.rpt
            }
        
        }
    }
    report_ocvm -type pocvm >> report_pocv_not_annotated.rpt
    #report_ocvm -type pocvm -coefficient -list_not_annotated -nosplit >> report_pocv_not_annotated.rpt
    report_ocvm -type pocvm -list_not_annotated -nosplit              >> report_pocv_not_annotated.rpt
    ...
    foreach voltage $VOLTAGE {
        if { [info exist aocvLibrary($voltage)] && $aocvLibrary($voltage) != "" } {
            foreach file $aocvLibrary($voltage) {
                echo "Read aocvm $file"    >> check_aocvm.log
                read_aocvm $file >> check_aocvm.log
            }
        }
    echo "Read aocvm $aocvWireFile" >> check_aocvm.log
    read_aocvm $aocvWireFile >> check_aocvm.log
    report_aocvm >> check_aocvm.log
    report_aocvm -list_not_annotated -nosplit > report_aocv_not_annotated.rpt
...
report_timing_derate -increment > timing_derate_incr.rpt
...
foreach voltage [array names pinList] {
    ...
    for {set i 0} {$i<[llength $clist]} {incr i} {
        ...
        #echo "set_max_transition -force $max_tran $pin_name" >> maxTransConstraint.rpt
    }
