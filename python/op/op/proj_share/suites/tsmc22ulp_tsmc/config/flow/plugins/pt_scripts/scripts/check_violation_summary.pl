...
while( <IN> ) {
  ...
  } elsif (/internal path startpoint/) {
      $startpoint_clock = "internal_path" ;
      $startpoint = "$startpoint_pin:$startpoint_clock" ;
  } elsif (/internal path endpoint/) {
      $endpoint_clock = "internal_path" ;
      $endpoint = "$endpoint_pin:$endpoint_clock" ;
      $clock_group = "$startpoint_clock,$endpoint_clock" ;
      if( $worst_slack_per_clock_group{$clock_group} eq "" ) {
         $worst_slack_per_clock_group{$clock_group} = 999999 ;
...
#     ( $endpoint_derating_clock_network_delay ) = /^\**\s*clock (network|tree) delay \(propagated|ideal\)\s+(\S+)\s+/ ;
  ...
  if( $is_data_path == 0 ) {
    if( ( $pin eq $startpoint_pin ) || ( $pin =~ /^($startpoint_pattern)\// ) ) {
      ...
      ##### ONLY FOR Bliss #####
      } elsif( $design eq "lm_top" ) {
        if( $startpoint_pin !~ /\// ) {
          $startpoint_block = "in" ;
        } elsif( $startpoint_pin =~ /u_scrypt\/g_SCRYPT_GROUP_(\d)__u_scrgrp\/g_SMIX_ENG_(\d)__u_seng\// ) {
          $startpoint_block = "SCRYPT_GROUP_${1}_SMIX_ENG_${2}" ;
        } elsif( $startpoint_pin =~ /u_scrypt\/g_SCRYPT_GROUP_(\d)__u_scrgrp\// ) {
          $startpoint_block = "SCRYPT_GROUP_${1}" ;
        } else {
          $startpoint_block = "top" ;
        }
      ##### ONLY FOR E-chip #####
      } elsif( $design eq "D4145" ) {
        if( $startpoint_pin !~ /\// ) {
          $startpoint_block = "in" ;
        } elsif( $startpoint_pin =~ /\/VDH264\// ) {
          $startpoint_block = "VDH264" ;
        } elsif( $startpoint_pin =~ /\/DDR3TOP\// ) {
          $startpoint_block = "DDR3TOP" ;
        } elsif ($startpoint_pin =~ /\/ARM1176JZFSWRP\//) {
          $startpoint_block = "ARM1176JZFSWRP" ;
        } else {
          $startpoint_block = "*TOP*" ;
        }
      ##### ONLY FOR DJIN5IO #####
      } elsif( $design eq "DJIN5IO" ) {
        if( $startpoint_pin !~ /\// ) {
          $startpoint_block = "in" ;
      ...
      } elsif( ( $design eq "t2cd" ) || ( $design eq "T2CD" ) || ( $design eq "T2CDS" ) ) {
        $startpoint_block = $startpoint_pin ;
        if( $startpoint_pin !~ /\// ) { $startpoint_block = "in" }
      ...
      if ( $module_map =~ /\S+/ ) {
        if ( $design eq $TOP ) {
          if( $startpoint_pin !~ /\// ) {
            ...
            $startpoint_block = "*TOP*";
            @module_map = split /\s+/, $module_map;
            foreach $map ( @module_map ) {
              my @name_map = split /\-/, $map;
              $instance_name = $name_map[0];
              $alias_name    = $name_map[1];
              if ( $startpoint_pin =~ m#$instance_name# ) {
                $startpoint_block = "$alias_name";
                last;
              }
            }
          }
  ...
  if( ( ( /^\**\s*data arrival time / ) && ( $endpoint_pin eq "" ) ) || ( ( $is_magma == 1 ) && ( /Reference clock path/ ) ) ) {
    ...
    ##### ONLY FOR Bliss #####
      ...
      if( $endpoint_pin !~ /\// ) {
        $endpoint_block = "out" ;
      } elsif( $endpoint_pin =~ /u_scrypt\/g_SCRYPT_GROUP_(\d)__u_scrgrp\/g_SMIX_ENG_(\d)__u_seng\// ) {
        $endpoint_block = "SCRYPT_GROUP_${1}_SMIX_ENG_${2}" ;
      } elsif( $endpoint_pin =~ /u_scrypt\/g_SCRYPT_GROUP_(\d)__u_scrgrp\// ) {
        $endpoint_block = "SCRYPT_GROUP_${1}" ;
      } else {
        $endpoint_block = "top" ;
      }
    ##### ONLY FOR DJIN5IO #####
     ...
     #  $endpoint_block = "DCOFDM1" ;
      ...
      ##### ONLY FOR E-chip #####
        ...
        if( $endpoint_pin !~ /\// ) {
          $endpoint_block = "out" ;
        } elsif( $endpoint_pin =~ /\/VDH264\// ) {
          $endpoint_block = "VDH264" ;
        } elsif( $endpoint_pin =~ /\/DDR3TOP\// ) {
          $endpoint_block = "DDR3TOP" ;
        } elsif ($endpoint_pin =~ /\/ARM1176JZFSWRP\//) {
          $endpoint_block = "ARM1176JZFSWRP" ;
        } else {
          $endpoint_block = "*TOP*" ;
        }
    ##### ONLY FOR MCTEG0 #####
      ...
      if ( $design eq $TOP ) {
        if( $endpoint_pin !~ /\// ) {
          ...
          foreach $map ( @module_map ) {
            ...
            $instance_name = $name_map[0];
            $alias_name    = $name_map[1];
            if ( $endpoint_pin =~ m#$instance_name# ) {
              $endpoint_block = "$alias_name";
              last;
            }
          }
        }
