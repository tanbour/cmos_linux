...
{#
    # set_app_options -list {time.pocvm_enable_constraint_variation true}
-#}
...
###==================================================================##
## Setting 7nm foundry specific settings                             ##
##===================================================================##
# settings.common.opt.tcl.7nm_t----------------------------------------
...
# settings.common.routing.tcl.7nm_t-----------------------------------

set 7nm_M0 [get_attribute [get_layers -filter {mask_name==metal1}] name]
...
{#-      # Specify for each layer whether to connect to pins by using vias or wires contained within the pin shapes
        #  "set_technology -node 7" automatically sets route.common.connect_within_pins_by_layer_name with 
        #  -value { {M0 via_wire_all_pins } {M1 off } }.
        #  For M2, via_wire_standard_cell_pins is recommended. 
        #  However, check with your library provider to see if it is recommended for your libraries.
        #  Uncomment below if you want to change it or to set M2 to via_wire_standard_cell_pins:
        ...
        #	] ;# name: M0, M1, M2 (mask_name: metal1, metal2, metal3)
        ...
        #  It is recommended to use tool default and not to reserve additional utilization, such as setting the value below 70.
        
        ## Set the cost multiplier for all via layers
        #  "set_technology -node 7" automatically sets it to 1 for all via layers.
        #  To change that, follow the example below and replace the number 1 in "%s 1" with a different integer.
        #	redirect -var x {set all_via_layers [get_layers -quiet -filter "mask_name =~ via*"]}
        #	if !{[regexp "Error" $x]} {
        #		set tmpStr1 ""
        #		foreach_in_collection mylay $all_via_layers {
        #		    set tmpStr [format "%s 1" [get_attribute -quiet -name name $mylay]]
        #		    lappend tmpStr1 $tmpStr
        #		}
        #		set_app_options -name route.common.extra_via_cost_multiplier_by_layer_name -value $tmpStr1
        #	}
        
        ## Set the non-preferred direction routing cost multiplier for all layers where y_default_width != x_default_width
        #  "set_technology -node 7" automatically sets it to 3 for all matched layers.
        #  To change that, follow the example below and replace the number 3 in "%s 3" with a different integer.
        #	redirect -var x {set mylayers [get_layers -quiet -filter "routing_direction != unknown"]}
        #	if !{[regexp "Error" $x]} {
        #		set tmpStr2 ""
        #		foreach_in_collection mylay $mylayers {
        #		    set xdw [get_attribute -quiet -name x_default_width $mylay]
        #		    set ydw [get_attribute -quiet -name y_default_width $mylay]
        #		    if { $xdw != "" && $ydw != "" && $xdw != $ydw } {
        #		        set tmpStr [format "%s 3" [get_attribute -quiet -name name $mylay]]
        #		        lappend tmpStr2 $tmpStr
        #		    }
        #		}
        #		if {$tmpStr2 != ""} {
        ...
        #	set_app_options -name route.common.enable_single_connection_for_var_width -value false ;# default true
        ...
        #  information.
        #  "set_technology -node 7" automatically sets "set_extraction_options" with "-honor_mask_constraints true" by default.
#}
...
# Modified by simonz  on:Mon Jun  4 13:13:31 CST 2018 : add legalize option from andrew for legalize issue
