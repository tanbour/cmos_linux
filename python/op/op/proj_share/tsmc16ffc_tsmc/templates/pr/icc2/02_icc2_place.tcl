...
write_verilog  -exclude {scalar_wire_declarations leaf_module_declarations pg_objects end_cap_cells well_tap_cells filler_cells pad_spacer_cells physical_only_cells cover_cells flip_chip_pad_cells} -hierarchy all {{env.RUN_DATA}}/{{env.BLK_NAME}}.${dst_stage}.pt.v 

## write_verilog for LVS (with pg, and with physical only cells)
write_verilog  -exclude {scalar_wire_declarations leaf_module_declarations empty_modules} -hierarchy all {{env.RUN_DATA}}/{{env.BLK_NAME}}.${dst_stage}.lvs.v

## write_verilog for Formality (with pg, no physical only cells, and no supply statements)
write_verilog  -exclude {scalar_wire_declarations leaf_module_declarations end_cap_cells well_tap_cells filler_cells pad_spacer_cells physical_only_cells cover_cells supply_statements} -hierarchy all {{env.RUN_DATA}}/{{env.BLK_NAME}}.${dst_stage}.fm.v 

## write_verilog for VC LP (with pg, no physical_only cells, no diodes, and no supply statements)
write_verilog -compress gzip -exclude {scalar_wire_declarations leaf_module_declarations end_cap_cells well_tap_cells filler_cells pad_spacer_cells physical_only_cells cover_cells diode_cells supply_statements} -hierarchy all {{env.RUN_DATA}}/{{env.BLK_NAME}}.${dst_stage}.vclp.v

{% if local.write_def_convert_icc2_site_to_lef_site_name_list != "" %} 
write_def -include_tech_via_definitions -convert_sites { $write_def_convert_icc2_site_to_lef_site_name_list } -compress gzip {{env.RUN_DATA}}/{{env.BLK_NAME}}.${dst_stage}.def
{%- else %}
write_def -include_tech_via_definitions -compress gzip {{env.RUN_DATA}}/{{env.BLK_NAME}}.${dst_stage}.def
{%- endif %}
{%- endif %}
####################################
## exit icc2
